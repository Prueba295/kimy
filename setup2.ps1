...
# Intento 1: Usando mc Docker image
try {
    docker run --rm --network=host --entrypoint sh minio/mc -c "
        mc alias set local http://localhost:9000 minioadmin minioadmin123 2>/dev/null
        mc mb local/thesis-documents --ignore-existing 2>/dev/null
    " 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) {
        Write-OK "Bucket 'thesis-documents' creado/verificado via mc"
        $bucketCreated = $true
    }
} catch {
...
$chromePath = "$env:LOCALAPPDATA\puppeteer\chrome"
