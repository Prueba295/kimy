...
import { Settings, Shield, Bell, User, Camera, LogOut, CheckCircle2, Loader2, Smartphone, Key, Copy, Download, AlertTriangle, Brain, Cpu, Zap, Sparkles } from 'lucide-react';
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { id: 'openai', name: 'OpenAI GPT-4o', icon: Sparkles, envVar: !!process.env.NEXT_PUBLIC_HAS_OPENAI },
                    { id: 'deepseek', name: 'DeepSeek V3', icon: Zap, envVar: false },
                    { id: 'gemini', name: 'Gemini 2.0 Flash', icon: Cpu, envVar: false },
                    { id: 'groq', name: 'Groq Llama 3', icon: Cpu, envVar: false },
                  ].map((p) => (
                    <label
                      key={p.id}
                    ...
                    >
                      <input
                        ...
                        name="aiProvider"
                        value={p.id}
                        checked={formData.aiProvider === p.id}
                        onChange={handleInputChange}
                        className="sr-only"
                      />
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        formData.aiProvider === p.id ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500'
                      }`}>
