...
export default function AdvanceDetail() {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      await api.get(`/reports/advance/${id}/send-email`);
      alert('El acta de revisión en PDF ha sido enviada exitosamente al correo del estudiante.');
    } catch (error) {
      alert('Error al enviar el correo con el reporte.');
    } finally {
      ...
      {tab==='plagiarism'&&<div className="space-y-4">
        {d.plagiarismReports?.[0] ? (
          <>
            <div className="bg-white rounded-xl border p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`text-2xl font-bold ${d.plagiarismReports[0].overallScore > 30 ? 'text-red-600' : d.plagiarismReports[0].overallScore > 15 ? 'text-amber-600' : 'text-green-600'}`}>
                  {d.plagiarismReports[0].overallScore.toFixed(1)}% <span className="text-xs font-normal text-gray-400">similitud total</span>
                </div>
              </div>
              <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${d.plagiarismReports[0].overallScore > 30 ? 'bg-red-500' : d.plagiarismReports[0].overallScore > 15 ? 'bg-amber-500' : 'bg-green-500'}`} style={{width: `${Math.min(d.plagiarismReports[0].overallScore, 100)}%`}}/>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border p-5 text-center">
            ...
            <p className="text-sm text-gray-500">No hay reporte de plagio aún</p>
            <button disabled={loading==='Plagio'} onClick={()=>handleAction('Plagio', `/plagiarism/analyze/${id}`)} className="mt-3 px-4 py-2 rounded-lg bg-primary-500 text-white text-xs font-medium disabled:opacity-50">{loading==='Plagio'?'Analizando...':'Ejecutar análisis'}</button>
          </div>
        )}
      </div>}
