...
export class AiAnalysisService {
  ...
  private buildPipeline(model?: string, provider?: string): AnalysisPipeline {
    ...
    const openaiKey = activeProvider === 'groq' 
  ...
  async analyzeAdvance(advanceId: string) {
    ...
    if (!hasOpenAI && !hasDeepSeek && !hasGemini && !hasGroq) {
    ...
    if (activeConfig.provider === 'deepseek' && hasDeepSeek) {
      try {
        ...
        result = await this.executePipeline(pipeline, advanceId);
        usedModel = `deepseek/${activeConfig.model}`;
      } catch (error: any) {
        this.logger.error(`DeepSeek analysis failed for ${advanceId}: ${error?.message}`);
      }
      ...
      try {
        ...
        result = await this.executePipeline(groqPipeline, advanceId);
      } catch (error: any) {
        this.logger.error(`Groq analysis failed for ${advanceId}: ${error?.message}`);
      }
      ...
      try {
        ...
        result = await this.executePipeline(geminiPipeline, advanceId);
      } catch (error: any) {
        this.logger.error(`Gemini analysis failed for ${advanceId}: ${error?.message}`);
      }
    } else if (hasOpenAI) {
      ...
      try {
        ...
        this.logger.error(`OpenAI analysis failed for ${advanceId}: ${error?.message}`);
      }
    }
    ...
    if (!result && hasDeepSeek && activeConfig.provider !== 'deepseek') {
      ...
      try {
        ...
        result = await this.executePipeline(dsPipeline, advanceId);
      } catch (dsError: any) {
        this.logger.error(`DeepSeek fallback failed for ${advanceId}: ${dsError?.message}`);
      }
    }
    ...
    if (!result && hasOpenAI && activeConfig.provider !== 'openai') {
      ...
      try {
        ...
        this.logger.error(`OpenAI fallback failed for ${advanceId}: ${openaiError?.message}`);
      }
    }
    ...
    if (!result && hasGroq) {
      ...
      try {
        ...
        this.logger.error(`GROQ fallback failed for ${advanceId}: ${groqError?.message}`);
      }
    }
    ...
    if (!result && hasGemini) {
      ...
      try {
        ...
        this.logger.error(`GEMINI fallback failed for ${advanceId}: ${geminiError?.message}`);
      }
    }
  ...
  private async executePipeline(pipeline: AnalysisPipeline, advanceId: string) {
  ...
  async generateDetailedFeedback(advanceId: string) {
    ...
    let usedModel = process.env.AI_MODEL || 'gpt-4o';

    if (analysis.modelUsed) {
      if (analysis.modelUsed.startsWith('groq/')) {
        usedProvider = 'groq';
        usedModel = analysis.modelUsed.split('/')[1];
      } else if (analysis.modelUsed.startsWith('deepseek/')) {
        usedProvider = 'deepseek';
        usedModel = analysis.modelUsed.split('/')[1];
      } else if (analysis.modelUsed.startsWith('gemini')) {
        usedProvider = 'gemini';
        usedModel = analysis.modelUsed;
      } else if (analysis.modelUsed.startsWith('openai/')) {
        usedProvider = 'openai';
        usedModel = analysis.modelUsed.split('/')[1];
      }
    }

    const pipeline = this.buildPipeline(usedModel, usedProvider);
