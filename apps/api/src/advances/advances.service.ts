...
export class AdvancesService {
  ...
  async bulkUpload(
    ...
    data: { templateId: string; advanceType: string; studentId: string },
  ) {
    const template = await this.prisma.thesisTemplate.findUnique({
      where: { id: data.templateId },
      select: { programId: true },
    ...
    for (const file of files) {
      ...
      const existingVersions = await this.prisma.advance.count({
        where: { studentId: data.studentId, advanceType: data.advanceType },
      });
      ...
      await Promise.all([
        this.aiAnalysisService.enqueueAnalyze(advance.id),
        this.plagiarismService.enqueueAnalyze(advance.id),
        this.referencesService.enqueueAnalyze(advance.id),
      ]);
