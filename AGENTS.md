...
1. Intenta el proveedor configurado (DB `SystemSettings.aiProvider` o env `AI_PROVIDER`)
2. Si falla → DeepSeek (si no fue el primario)
3. Si falla → OpenAI (fallback genérico)
4. Si falla → Groq
5. Si falla → Gemini
6. Si TODO falla → **Simulación** (datos mock)
...
| Env Var | Provider |
|---------|----------|
| `OPENAI_API_KEY` | OpenAI + Groq (Groq usa esta key) |
| `DEEPSEEK_API_KEY` | DeepSeek (fallback a OPENAI_API_KEY) |
| `GEMINI_API_KEY` | Gemini |
| `GROQ_API_KEY` | Groq (usada como OPENAI_API_KEY en pipeline Groq) |
| `AI_PROVIDER` | `openai` \| `groq` \| `gemini` \| `deepseek` |
